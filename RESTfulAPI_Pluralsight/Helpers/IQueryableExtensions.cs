...
namespace RESTfulAPI_Aync.Helpers
{
    public static class IQueryableExtensions
    {
        public static IQueryable<T> ApplySort<T>(this IQueryable<T> source, string orderBy ,Dictionary<string, PropertyMappingValue> mappingDictionary)
        {
            ...
            //the IQueryable will be ordered in the wrong order
            foreach(var orderByClause in orderByAfterSplit.Reverse())
            {
                //trim the orderby clause
                var trimmedOrderByclause = orderByClause.Trim();

                var orderDescending = trimmedOrderByclause.EndsWith(" desc");

                //remove " asc" or " desc" from orderBy clause, so we get the propperty name to look for in the mapping dictionary
                ...
                //actual sorting using Dynamic Link
